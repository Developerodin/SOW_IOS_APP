...
const MarketModal = ({
  ...
  const getHistoryByTimeframe = async (mandiId, category, timeframe) => {
    try {
      const response = await axios.get(
        `${Base_url}api/mandi_rates/history/timeframe/${mandiId}/${category}/${timeframe}`
      );
  ...
  const addMandiToList = async (mandiId, listType) => {
    try {
      ...
      if (error.response && error.response.status === 404) {
        console.error(
          `Error 404: Not Found. Please check userId, mandiId, or listType:`,
          error
        );
      } else {
  ...
  const fetchUserMandis = async (userId, mandiId) => {
    try {
      ...
      const favoriteMandis =
        response.data.favoriteMandis?.map((mandi) => mandi._id) || [];
      const notificationMandis =
        response.data.notificationFormMandiList?.map((mandi) => mandi._id) ||
        [];
    ...
    if (modalVisible) {
      const fetchHistory = async () => {
        try {
          ...
          const historyData = await getHistoryByTimeframe(
            mandiId,
            category,
            timeframe
          );
          ...
          if (historyData.length === 0) {
            ...
            let processedData = historyData
              ...
              .filter(
                (entry) => !isNaN(entry.price) && !isNaN(entry.date.getTime())
              );
  ...
  const processYearlyData = (data) => {
    ...
    return Object.keys(groupedByMonth).map((month) => ({
      ...
      price: Math.round(
        groupedByMonth[month].price / groupedByMonth[month].count
      ), // Rounded average price per month
    }));
  };
  ...
  const { highestPrice, lowestPrice, averagePrice } =
    ...
    <View style={[styles.buttonGroup]}>
      ...
      >
        <View style={styles.iconTextContainer}>
          <Ionicons
            name={
              notificationAdded.includes(selectedItem?.mandi?._id)
      ...
      >
        <View style={styles.iconTextContainer}>
          <Ionicons
            name={
              favouriteAdded.includes(selectedItem?.mandi?._id)
                ? "checkmark-circle-outline"
                : "heart-outline"
            }
            size={20}
            ...
            {loading ? (
              ...
              <>
                <Text>
                  ...
                  {["Today", "Week", "Month", "Year"][activeButton]}
                </Text>
                <Image
              ...
              <>
                    ...
                    <LineChart
                      data={{
                        labels: priceHistory.map((entry) =>
                          formatLabel(new Date(entry.date), timeframe)
                        ),
                        datasets: [
                          {
                            data: priceHistory.map((entry) => entry.price),
                            color: (opacity = 1) =>
                              `rgba(0, 115, 177, ${opacity})`,
                            strokeWidth: 2,
                          },
                        ],
                      }}
                ...
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <TouchableOpacity
                      onPress={() => console.log(`Item pressed: ${item.date}`)}
                    >
                      <View style={styles.infoRow}>
                        <Text style={styles.infoRowText}>
                          {formatDate(item.date)}
                        </Text>
                        <Text style={styles.infoRowTextRight}>
                          ₹ {item.price}
                        </Text>
                      </View>
                    </TouchableOpacity>
